...
export class FileController {
  ...
  async savaUserAvatar(
    @Req() req: IUserReq,
    @UploadedFile() avatar: Express.Multer.File,
  ) {
    const filePath = path.resolve(
      'src/../.uploads',
      String(req.user.id) + String(avatar.fieldname),
    )
    ...
    fs.rmSync(filePath)
    const avatarTable = await this.FileService.saveAvatar(
      Location,
      avatar.mimetype,
      String(avatar.size) + 'bit',
    )
  ...
  async savaMomentPictures(
    ...
    for (const item of files) {
      const filePath = path.resolve(
        'src/../.uploads',
        String(req.user.id) + createPicName('pic-', '.png'),
      )
    ...
    const momentTables = await this.MomentService.getMomentByUserIdAndMomentId(
      req.user.id,
      momentId,
    )
    ...
    const userTable = (await this.UserService.findByName(
      req.user.username,
    )) as UserTable
    console.log(
      await this.FileService.saveMomentPics(
        userTable,
        momentTables[0],
        fileArr,
      ),
